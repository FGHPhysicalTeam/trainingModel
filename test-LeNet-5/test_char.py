...
with tf.Session() as sess:
    ...
    for i in range(0, 20):
        # 需要修改
        path = "D:\\pycharmProjects\\test-lenet-5_2\\test-LeNet-5\\Pic\\testPicChar\\%s\\" % i
        # path = "/Users/fengyushan/Desktop/22/"
        for rt, dirs, files in os.walk(path):
            for filename in files:

                path = "D:\\pycharmProjects\\test-lenet-5_2\\test-LeNet-5\\Pic\\testPicChar\\%s\\" % i + filename
                # path = "/Users/fengyushan/Desktop/22/" + filename
                # if path != "/Users/fengyushan/Desktop/22/.DS_Store":
                if path != "D:\\pycharmProjects\\test-lenet-5_2\\test-LeNet-5\\Pic\\testPicChar\\%s\\.DS_Store" % i:
                    print(filename+"  ")
                    img = Image.open(path)
                    ...
                    if (LETTERS_DIGITS[max1_index][0] == filename[0]) or (LETTERS_DIGITS[max1_index][0] == "θ" and filename[0] == "胃"):
                        count = count + 1
                    else:
                        print("wrong!!!")
